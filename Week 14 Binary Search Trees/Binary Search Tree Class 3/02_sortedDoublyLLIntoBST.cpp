...
Node *convertBSTtoDLL(Node *&head, int n) // by reference bhul gye the
{
    // Base case
    if (head == NULL || n <= 0)
    {
    ...
    // LNR
    // L
    Node *leftSubtree = convertBSTtoDLL(head, n / 2);

    // N
    Node *root = head;
    root->left = leftSubtree;
    // IMP : head update //bhul jate h bar bar
    head = head->right;

    // R
    Node *rightSubtree = convertBSTtoDLL(head, n - n / 2 - 1);
...
int main()
{
    ...
    Node *root = convertBSTtoDLL(head, 3);
